...
export class Phase9Step7DebugRunner {
  ...
  private async _test_02_repository(): Promise<void> {
    ...
    this._assert(ch1!.stages.length === 6, 'T02.015', 'chapter_001 应有 6 个关卡');
    ...
    this._assert(stages.length === 6, 'T02.024', 'getStagesByChapter 返回 6 关');

    // getStageCount
    this._assert(repository.getStageCount('chapter_001') === 6, 'T02.025', 'chapter_001 关卡数 6');
    ...
    this._assert(last!.stageIndex === 6, 'T02.031', '末关 index 为 6');
    this._assert(last!.type === 'boss', 'T02.032', '末关类型为 boss');
    ...
    const lastNext = repository.getNextStage('chapter_001_stage_06');
    this._assert(lastNext === null, 'T02.035', '末关的下一关为 null');
    ...
    this._assert(power >= 500, 'T02.038', 'chapter_001 推荐战力 >= 500');
    ...
    // 验证 boss 关卡
    const bossStage = repository.getStage('chapter_001_stage_06');
    this._assert(bossStage!.bossId !== '', 'T02.042', 'boss 关卡应有 bossId');
    this._assert(bossStage!.type === 'boss', 'T02.043', 'boss 关卡类型为 boss');
  ...
  private async _test_07_chapter_completion_and_chain(): Promise<void> {
    ...
    // 完成最后一关（boss 关）
    const resultBoss = system.completeStage('chapter_001_stage_06');
    this._assert(resultBoss, 'T07.001', '完成 boss 关卡成功');

    // 章节完成
    this._assert(system.isChapterCompleted('chapter_001'), 'T07.002', '章节已完成');

    const prog1 = system.getChapterProgress('chapter_001');
    this._assert(prog1!.status === 'completed', 'T07.003', '进度状态为 completed');
    this._assert(prog1!.completedAt > 0, 'T07.004', '完成时间已设置');
    this._assert(prog1!.completedStageIds.length === 6, 'T07.005', '全部 6 关已完成');

    // 章节完成事件
    const chapterCompleteEvents = this._getEventsOfType(ChapterSystem.CHAPTER_COMPLETED);
    this._assert(chapterCompleteEvents.length >= 1, 'T07.006', '应有章节完成事件');
    this._assert(
    ...
    const completeEvents = this._getEventsOfType(ChapterSystem.STAGE_COMPLETED);
    const bossEvent = completeEvents.find((e) => e.stageId === 'chapter_001_stage_06');
    this._assert(bossEvent !== undefined, 'T07.008', 'boss 关完成事件存在');
    this._assert(bossEvent!.isChapterComplete === true, 'T07.009', 'boss 关完成时 isChapterComplete 为 true');

    // 第二章应自动解锁
    this._assert(system.isChapterUnlocked('chapter_002'), 'T07.010', '第二章自动解锁');
    ...
    this._assert(ch2Event !== undefined, 'T07.011', '第二章解锁事件已发送');
    ...
    this._assert(ch2Stage1 !== undefined, 'T07.012', '第二章首关解锁事件已发送');
  }
  ...
  private async _test_08_snapshot(): Promise<void> {
    ...
    this._assert(ch1Snapshot!.completedStageCount === 6, 'T08.012', '第一章已完成 6 关');
  ...
  private async _test_09_save_restore(): Promise<void> {
    ...
    this._assert(savedCh1.completedStageIds.length === 6, 'T09.008', 'chapter_001 已完成 6 关');
    ...
    this._assert(prog1!.completedStageIds.length === 6, 'T09.015', '恢复后已完成 6 关');
  ...
  private async _test_10_edge_cases(): Promise<void> {
    ...
    const bossStage = repository.getStage('chapter_001_stage_06');
    this._assert(bossStage!.type === 'boss', 'T10.010', 'stage_06 类型为 boss');
  ...
  private async _test_11_getCurrentChapter(): Promise<void> {
    ...
    this._assert(ch1Stage!.id === 'chapter_001_stage_06', 'T11.006', '已完成章节的当前关卡为最后一关');
  ...
  private async _test_12_repository_queries(): Promise<void> {
    ...
    this._assert(ch1Power === 500, 'T12.07', 'chapter_001 推荐战力为 500');
